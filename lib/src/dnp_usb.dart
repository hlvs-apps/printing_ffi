...
/// A DNP/Citizen USB dye-sub printer the plugin has detected and (optionally) opened.
@immutable
class DnpUsbPrinter {
  const DnpUsbPrinter({
...
///
/// Owned by the plugin: [PrintingFfi.instance.initializeAndroidCups] starts this
/// once the in-app cupsd is up. Apps observe [PrintingFfi.instance.dnpPrinters]
/// (which mirrors [printers]) and drive printing via the detected [DnpUsbPrinter.queueName].
///
/// ANR NOTE: the add flow ([_onOpened]) runs SYNCHRONOUS FFI (startUsbFdServer +
/// addCupsPrinter) on the calling (UI/root) isolate and MUST stay there — libcups
/// keeps the target cupsd in per-thread `cupsSetServer` state, so these calls
/// cannot run on a background isolate (they would miss the in-app cupsd). The
/// initial scan is deferred a beat after first frame ([_initialScanDelay]) to keep
/// this synchronous work off the startup focus-acquisition window (whose starvation
/// otherwise causes "Waited 10000ms for FocusEvent" ANRs). See also the C14 note on
/// [PrintingFfi.initializeAndroidCups].
class DnpUsb {
  DnpUsb._();
