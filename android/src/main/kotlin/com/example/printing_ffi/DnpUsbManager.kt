 ...
 * connection.
 *
 * Lifecycle split (owned by [PrintingFfiPlugin], a FlutterPlugin + ActivityAware):
 *  - Constructed + [attachChannels] at engine attach: channels are engine-scoped and
 *    the USB work (executor + open connections) survives an Activity config change so
 *    an in-flight print is not killed on rotation/backgrounding (see C11).
 *  - [registerReceivers] at Activity attach (a Context is enough; the app context is
 *    used so the receivers outlive the Activity across config changes).
 *  - [unregisterReceivers] on a REAL Activity detach only.
 *  - [shutdown] at engine detach: closes connections + the executor.
 */
class DnpUsbManager(
    private val context: Context,
    methodMessenger: BinaryMessenger,
) {
    private val tag = "PrintingFfiUsb"
    private val usbManager: UsbManager =
        context.getSystemService(Context.USB_SERVICE) as UsbManager

    // C8: app-scoped permission action derived from the package name, so two apps
    // embedding this plugin can't collide on a shared, hard-coded action string.
    private val actionUsbPermission = "${context.packageName}.PRINTING_FFI_USB_PERMISSION"
    ...
    // Mirrors res/xml/printing_ffi_usb_device_filter.xml. Keep in sync.
    ...
    /**
     * Wire the MethodChannel + EventChannel. Engine-scoped: called once at
     * onAttachedToEngine, so the USB stack (channels, executor, connections) survives
     * an Activity config change and doesn't kill an in-flight print (C11).
     */
    fun attachChannels() {
        methodChannel.setMethodCallHandler { call, result ->
            when (call.method) {
                ...
                "startForegroundService" -> {
                    ...
                    DnpUsbForegroundService.start(context, text)
                    result.success(true)
                }
                "stopForegroundService" -> {
                    DnpUsbForegroundService.stop(context)
                    result.success(true)
                }
    ...
    /**
     * Register the hotplug + permission BroadcastReceivers. Activity-scoped (needs a
     * Context) but registered against the application context so they keep working
     * across an Activity config change; idempotent.
     */
    ...
    /** Unregister the receivers on a REAL Activity detach (NOT a config change). */
    fun unregisterReceivers() {
        if (!receiversRegistered) return
        receiversRegistered = false
        try { context.unregisterReceiver(attachDetachReceiver) } catch (_: Throwable) {}
        try { context.unregisterReceiver(permissionReceiver) } catch (_: Throwable) {}
    }

    /**
     * Full teardown at engine detach: drop the channels, receivers, all open USB
     * connections, and the executor.
     */
    fun shutdown() {
        unregisterReceivers()
     ...
     *  Called on the main thread (onAttachedToActivity / onNewIntent); must NOT do
